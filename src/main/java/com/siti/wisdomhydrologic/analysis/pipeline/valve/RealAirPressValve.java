...
public  class RealAirPressValve implements Valve<RealVo,Real,APEntity>,ApplicationContextAware {
    ...
    public void beforeProcess(List <RealVo> realData) {
        logger.info( new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"进入气压分析!");
    ...
    public void doProcess(List <RealVo> realData,List<Real> previousData,Map <Integer, APEntity> configMap) {
        try {
            ...
            logger.info("气压数据预计到达数量"+configMap.size()+",实际到达数量"+mapval.size());

            Iterator<Map.Entry<Integer, APEntity>> iterator = configMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<Integer, APEntity> entry = iterator.next();

                try {
                    RealVo vo = mapval.get(entry.getKey());  // 元素数据
                    APEntity config = entry.getValue();  // 元素配置
