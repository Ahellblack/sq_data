...
import com.siti.wisdomhydrologic.analysis.entity.FVEntity;
 ...
 */
@Component
public  class RealFlowVelocityValve implements Valve<RealVo,FVEntity,Real>,ApplicationContextAware {
    ...
    @Override
    public void doProcess(Map<Integer, RealVo> mapval, Map<Integer, FVEntity> configMap, LocalDateTime time,
        ...
        try {
                ...
                if (vo != null) {
                    ...
                    if (realvalue < config.getLevelMin()) {
                        exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                ...
                                .dataError( DataError.LESS_FlowVelocity.getErrorCode() )
                        ...
                        exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                ...
                                .sensorCode( vo.getSenId() )
                                .errorValue( realvalue )
                                .dataError( DataError.MORE_FlowVelocity.getErrorCode() )
                                .build() );
                    }
                    //---------------------------变化率分析-------------------------
                    if (!flag) {
                        ...
                        if (real != null) {
                            ...
                            if (frant.subtract( end ).doubleValue() > config.getUpMax()) {
                                exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                        ...
                                        .dataError( DataError.UP_MAX_FlowVelocity.getErrorCode() )
                                        .build() );
                                flag = true;
                            } else {
                                if (frant.subtract( end ).doubleValue() < config.getBelowMin()) {
                                    exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                            ...
                                            .sensorCode( vo.getSenId() )
                                            .errorValue( vo.getFACTV() )
                                            .dataError( DataError.DOWN_MAX_FlowVelocity.getErrorCode() )
                                            .build() );
                                    flag = true;
                                }
                            }
                        ...
                        if(config.getDuration() / 5<=times[0]){
                            exceptionContainer[0].add(new AbnormalDetailEntity.builer()
                                    .date(LocalDateUtil
                                            ...
                                            .format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss")))
                                    .sensorCode(vo.getSenId())
                                    .dataError(DataError.DURING_FlowVelocity.getErrorCode())
                                    .build());
                        }
                        flag=true;
                    }
                    ...
                    exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                            .date( date )
                            .sensorCode( config.getSensorCode() )
                            .dataError( DataError.BREAK_FlowVelocity.getErrorCode() )
                            .build() );
                }
            } );
            ...
            logger.error( "RealFlowVelocityValve异常：{}", e.getMessage() );
        }
    }
