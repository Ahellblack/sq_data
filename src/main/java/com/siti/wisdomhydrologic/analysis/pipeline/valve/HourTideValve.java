...
@Component
public class HourTideValve implements Valve<DayVo, TideLevelEntity, Real>, ApplicationContextAware {
    ...
    public void beforeProcess(List<DayVo> realList, Map<String, Real> compare) {
        ...
        Map <Integer, TideLevelEntity> tideLevelMap = Optional.of( abnormalDetailMapper.fetchAllT() )
                .get()
                .stream()
                .collect(Collectors.toMap(TideLevelEntity::getSensorCode, a -> a));
        //--------------------筛选出实时数据-----------------------------
    ...
    @Override
    public void doProcess(Map<Integer, DayVo> mapval, Map<Integer, TideLevelEntity> configMap, LocalDateTime time
        ...
        try {
            ...
            Map <String, Real> maps = compare.keySet().stream().filter(
                    e -> (e.split( "," )[1].contains(  ConstantConfig.TS+ "" ))
                    ...
                    if (vo != null) {
                        ...
                        if (vo.getMinV() < config.getLevelMin()) {
                            exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                    ...
                                    .dataError( DataError.HOUR_LESS_TIDE.getErrorCode() )
                                    .build());
                            flag=true;
                            ...
                            exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                    ...
                                    .dataError( DataError.HOUR_MORE_TIDE.getErrorCode() )
                                    .build() );
                            flag=true;
                        }
                        //---------------------------------回归模型分析--------------------------------
            ...
            logger.error( "HourTideValve异常：{}", e.getMessage() );

        }
    }

    @Override
    public void doProcess(Map<Integer, DayVo> mapval, Map<Integer, TideLevelEntity> configMap) {
