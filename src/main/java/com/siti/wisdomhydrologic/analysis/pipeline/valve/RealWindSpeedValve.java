...
public class RealWindSpeedValve implements Valve<RealVo, Real, WSEntity>, ApplicationContextAware {
    ...
    public void beforeProcess(List<RealVo> realData) {
        logger.info( new SimpleDateFormat("yyyy-MM-dd HH:mm:ss").format(new Date())+"进入风速分析!");
    ...
    public void doProcess(List<RealVo> realData, List<Real> previousData, Map<Integer, WSEntity> configMap) {
        try {
            ...
            logger.info("风速数据预计到达数量" + configMap.size() + ",实际到达数量" + mapval.size());

            Iterator<Map.Entry<Integer, WSEntity>> iterator = configMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<Integer, WSEntity> entry = iterator.next();

                try {
                    RealVo vo = mapval.get(entry.getKey());  // 元素数据
                    WSEntity config = entry.getValue();  // 元素配置
                    ...
                    if(!flag){
                        ...
                        if (regressionFunc.size() <= 0) {  // 回归模型不存在
                            ...
                            if (flag){
                                dataErrorCode = DataError.REGRESSION_EXCEPTION_WINDSPEED.getErrorCode();
                            }
                        }
                    }  // 回归模型分析结束
