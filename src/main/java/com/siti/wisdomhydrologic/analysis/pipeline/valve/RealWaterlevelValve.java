...
public class RealWaterlevelValve implements Valve<RealVo, Real, WaterLevelEntity>, ApplicationContextAware {
    ...
    public void beforeProcess(List<RealVo> realData) {
        ...
        //------------------读取real过去一段时间数据-----------------
        String before = LocalDateUtil.dateToLocalDateTime(realData.get(0).getTime()).minusHours(6).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
        List<Real> previousData = abnormalDetailMapper.selectBeforeFiveReal(before, ConstantConfig.WS);
        //----------------------获取配置表---------------------------
        Map<Integer, WaterLevelEntity> configMap = Optional.of(abnormalDetailMapper.fetchAllW()).get().stream().collect(Collectors.toMap(WaterLevelEntity::getSensorCode, a -> a));
    ...
    public void doProcess(List<RealVo> realData, List<Real> previousData, Map<Integer, WaterLevelEntity> configMap) {
        try {
            ...
            if (previousData.size() > 0) {
                compareMap = previousData.stream().collect(Collectors.toMap((real) -> real.getTime() + "," + real.getSensorCode(), account -> account));
            }
            //--------------------筛选出实时数据-------------------------
            Map<Integer, RealVo> mapval = realData.stream().filter(e -> ((e.getSenId() % 100) == ConstantConfig.WS)).collect(Collectors.toMap(RealVo::getSenId, a -> a));
            ...
            String date = LocalDateUtil.dateToLocalDateTime(realData.get(0).getTime()).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));  // 记录时间
            logger.info("水位数据预计到达数量" + configMap.size() + ",实际到达数量" + mapval.size());

            Iterator<Map.Entry<Integer, WaterLevelEntity>> iterator = configMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<Integer, WaterLevelEntity> entry = iterator.next();

                try {
                    RealVo vo = mapval.get(entry.getKey());  // 元素数据
                    WaterLevelEntity config = entry.getValue();  // 元素配置
                    ...
                    if (!flag) {
                        if (vo == null) {  // MQ数据包数据不存在
                            dataErrorCode = DataError.BREAK_WATERLEVEL.getErrorCode();
                            flag = true;
                            descStr += "==>中断分析:数据发生中断！";
                            savedStr = "数据发生中断！";
                        } else {
                    ...
                    if (!flag) {
                        if (realvalue < config.getLevelMin()) {
                            dataErrorCode = DataError.LESS_WATERLEVEL.getErrorCode();
                            flag = true;
                            ...
                            dataErrorCode = DataError.MORE_WATERLEVEL.getErrorCode();
                            flag = true;
                    ...
                    if (!flag) {
                        if (null == config.getUpMax() || null == config.getDownMax()) {
                            ...
                            String before = LocalDateUtil.dateToLocalDateTime(realData.get(0).getTime()).minusMinutes(5).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));
                            ...
                            if (real != null) {
                                ...
                                if (result > config.getUpMax()) {
                                    dataErrorCode = DataError.UP_MAX_WATERLEVEL.getErrorCode();
                                    flag = true;
                                    ...
                                    dataErrorCode = DataError.DOWN_MAX_WATERLEVEL.getErrorCode();
                                    flag = true;
                    ...
                    if (!flag) {
                        if (null == config.getDuration() || "".equals(config.getDuration())) {
                            ...
                            String before = LocalDateUtil.dateToLocalDateTime(realData.get(0).getTime()).minusMinutes(config.getDuration()).format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm:ss"));

                            SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
                            List<Real> realList = previousData.stream().filter(real -> real.getSensorCode() == sensorCode).filter(real -> {
                                try {
                            ...
                            if (((float) realList.size() / needCount) < 0.8) {
                                ...
                                if (realList.stream().allMatch(element -> element.getRealVal().equals(realvalue))) {
                                    flag = true;
                                    dataErrorCode = DataError.DURING_WATERLEVEL.getErrorCode();
                    ...
                    //---------------------------回归模型分析-------------------------
                    if (!flag) {
                        List<RegressionEntity> regressionFunc = rlists.stream().filter(object -> object.getSectionCode().equals(sensorCode)).collect(Collectors.toList());
                            ...
                            if (regressionEntity.getRefNum() == 1) {
                                if (null == arg0 || null == redisualMax || null == arg1 || null == value1) {
                                    ...
                                    if (Math.abs(predictValue - realvalue) > redisualMax) {
                                        flag = true;
                                        descStr += "==>回归模型分析:残差过大，回归模型异常！" + "redisualMax < abs(predictValue - realvalue) = arg0 + value1 * arg1 " + redisualMax + " < abs(" + predictValue + " -" + realvalue + ") =" + arg0 + "+" + value1 + " *" + arg1;
                                        savedStr = "回归异常，预测值 " + predictValue + " 与实际值 " + realvalue + "相差超过" + redisualMax + "!";
                                    } else {
                                        descStr += ";回归模型分析得到==>残差正常，回归模型正常！" + "redisualMax < abs(predictValue - realvalue) = arg0 + value1 * arg1 " + redisualMax + " < abs(" + predictValue + " -" + realvalue + ") =" + arg0 + "+" + value1 + " *" + arg1;
                                    }
                                }
                                ...
                                if (null == arg0 || null == redisualMax || null == arg1 || null == value1 || null == arg2 || null == value2) {
                                    descStr += ";回归模型分析得到==>回归模型参数不全，跳过分析！" + regressionEntity.toString();
                                } else {
                                    Double predictValue = value1 * arg1 + value2 * arg2 + arg0;
                                    if (Math.abs(predictValue - realvalue) > redisualMax) {
                                        flag = true;
                                        descStr += "==>回归模型分析:残差过大，回归模型异常！" + "redisualMax < abs(predictValue - realvalue) = arg0 + value1 * arg1 + value2 * arg2" + redisualMax + " < abs(" + predictValue + " -" + realvalue + ") =" + arg0 + "+" + value1 + " *" + arg1 + "+" + value2 + " *" + arg2;
                                        savedStr = "元素" + sensorCode + ",回归异常，预测值 " + predictValue + " 与实际值 " + realvalue + "相差超过" + redisualMax + "!";
                                    } else {
                                        descStr += "==>回归模型分析:残差正常，回归模型正常！" + "redisualMax < abs(predictValue - realvalue) = arg0 + value1 * arg1 + value2 * arg2" + redisualMax + " < abs(" + predictValue + " -" + realvalue + ") =" + arg0 + "+" + value1 + " *" + arg1 + "+" + value2 + " *" + arg2;
                                    }
                                }
                                ...
                                if (null == arg0 || null == redisualMax || null == arg1 || null == value1 || null == arg2 || null == value2 || null == arg3 || null == value3) {
                                    descStr += "==>回归模型分析:回归模型参数不全，跳过分析！" + regressionEntity.toString();
                                } else {
                                    Double predictValue = arg0 + value1 * arg1 + value2 * arg2 + value3 * arg3;
                                    if (Math.abs(predictValue - realvalue) > redisualMax) {
                                        flag = true;
                                        descStr += "==>回归模型分析:残差过大，回归模型异常！" + "redisualMax < abs(predictValue - realvalue) = arg0 + value1 * arg1 + value2 * arg2" + redisualMax + " < abs(" + predictValue + " -" + realvalue + ") =" + arg0 + "+" + value1 + " *" + arg1 + "+" + value2 + " *" + arg2 + "+" + value3 + " *" + arg3;
                                        savedStr = "回归异常，预测值 " + predictValue + " 与实际值 " + realvalue + "相差超过" + redisualMax + "!";
                                    } else {
                                        descStr += "==>回归模型分析:残差正常，回归模型正常！" + "redisualMax < abs(predictValue - realvalue) = arg0 + value1 * arg1 + value2 * arg2" + redisualMax + " < abs(" + predictValue + " -" + realvalue + ") =" + arg0 + "+" + value1 + " *" + arg1 + "+" + value2 + " *" + arg2 + "+" + value3 + " *" + arg3;
                                    }
                                }
                            } else {
                    ...
                    if (flag) {  // 出现异常才往异常表添加数据
                        exceptionContainer[0].add(new AbnormalDetailEntity.builer().date(date).sensorCode(sensorCode).errorValue(realvalue).dataError(dataErrorCode).description(savedStr).build());
                    }
                } catch (Exception e) {
                    e.printStackTrace();
                }
