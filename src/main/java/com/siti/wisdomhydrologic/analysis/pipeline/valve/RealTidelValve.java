                ...
                if (vo != null) {
                    ...
                    if (realvalue < config.getLevelMin()) {
                        exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                ...
                                .dataError( DataError.LESS_TIDE.getErrorCode() )
                                .build() );
                        flag=true;
                        ...
                        exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                ...
                                .sensorCode( vo.getSenId() ).sensorCode( vo.getSenId() )
                                .errorValue( realvalue )
                                .dataError( DataError.MORE_TIDE.getErrorCode() )
                                .build() );
                        flag=true;
                    }
                    //---------------------------变化率分析-------------------------
                        ...
                        if (real != null) {
                            ...
                            if (frant.subtract( end ).doubleValue() > config.getUpMax()) {
                                exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                        ...
                                        .dataError( DataError.UP_MAX_TIDE.getErrorCode() )
                                        .build() );
                                flag = true;
                            } else {
                                if (frant.subtract( end ).doubleValue() < config.getBelowMin()) {
                                    exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                                            ...
                                            .sensorCode( vo.getSenId() )
                                            .errorValue( vo.getFACTV() )
                                            .dataError( DataError.DOWN_MAX_TIDE.getErrorCode() )
                                            .build() );
                                    flag = true;
                                }
                            }
                    ...
                    exceptionContainer[0].add( new AbnormalDetailEntity.builer()
                            .date( date )
                            .sensorCode( config.getSensorCode() )
                            .dataError( DataError.BREAK_TIDE.getErrorCode() )
                            .build() );
                }
            } );
