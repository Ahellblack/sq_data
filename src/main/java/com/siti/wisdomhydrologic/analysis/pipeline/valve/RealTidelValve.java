...
public class RealTidelValve implements Valve<RealVo, Real, TideLevelEntity>, ApplicationContextAware {
    ...
    public void doProcess(List<RealVo> realData, List<Real> previousData, Map<Integer, TideLevelEntity> configMap) {
        try {
            ...
            System.out.println("潮位数据预计到达数量" + configMap.size() + ",实际到达数量" + mapval.size());

            Iterator<Map.Entry<Integer, TideLevelEntity>> iterator = configMap.entrySet().iterator();
            while (iterator.hasNext()) {
                Map.Entry<Integer, TideLevelEntity> entry = iterator.next();

                RealVo vo = mapval.get(entry.getKey());  // 元素数据
                TideLevelEntity config = entry.getValue();  // 元素配置
                ...
                if (!flag) {
                    if (vo == null) {  // MQ数据包数据不存在
                        dataErrorCode = DataError.BREAK_TIDE.getErrorCode();
                        flag = true;
                        descStr += "==>中断分析:数据发生中断！";
                    } else {
                        descStr += "==>中断分析:通过！";
                    ...
                    if (null == config.getDownMax() || "".equals(config.getDownMax())) {
                        ...
                        if (real != null) {
                            ...
                            if (result > config.getUpMax()) {
                                dataErrorCode = DataError.UP_MAX_TIDE.getErrorCode();
                                flag = true;
                                descStr += "==>变化率分析:上升值超过最大值配置！" + end + " -" + frant + " =" + result + " >" + config.getUpMax();
                            } else if (result < config.getDownMax()) {
                                dataErrorCode = DataError.DOWN_MAX_TIDE.getErrorCode();
                                flag = true;
                ...
                if (!flag) {
                    if (null == config.getDuration() || "".equals(config.getDuration())) {
                        ...
                        if (((float) realList.size() / needCount) < 0.8) {
                            ...
                            if (allEqualsFlag) {
                                flag = true;
                                dataErrorCode = DataError.DURING_TIDE.getErrorCode();
                ...
                if (!flag) {
                    ...
                    if (regressionFunc.size() <= 0) {  // 回归模型不存在
                        ...
                        if (flag) {
                            dataErrorCode = DataError.REGRESSION_EXCEPTION_TIDE.getErrorCode();
                        }
                    }
                }  // 回归模型分析结束
            ...
            logger.error("RealTideValve异常：{}", e.getMessage());
        }
    }
