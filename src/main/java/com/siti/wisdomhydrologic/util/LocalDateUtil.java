...
public class LocalDateUtil {
    ...
    public static void main(String[] args) {
        Date date1=new Date();
        LocalDateTime d=dateToLocalDateTime(date1);
        String time= LocalDateUtil.dateToLocalDateTime(date1).plusHours(-1).plusMinutes((0+1)*5).format(DateTimeFormatter.ofPattern(FORMAT_PATTERN1));
        String of=time.substring(0,time.lastIndexOf(":"));
        // String a=localDateTimeFormat(time,FORMAT_PATTERN1);
        d=d.plusHours(-1);
        try {
            Thread.sleep(1000);
